...
void CameraAravisNodelet::onInit()
{
    ...
    for (int i = 0; i < num_streams_; i++)
    {
        ...
        if (awb_mode == "Off" && !wb_ratio_selector_feature.empty() && implemented_features_[wb_ratio_selector_feature] && !wb_ratio_feature.empty() && implemented_features_[wb_ratio_feature])
        {
    ...
    config_.AcquisitionMode =
      implemented_features_["AcquisitionMode"] ? arv_device_get_string_feature_value(p_device_, "AcquisitionMode") : "Continuous";
    ...
    config_.TriggerSource =
      implemented_features_["TriggerSource"] ? arv_device_get_string_feature_value(p_device_, "TriggerSource") : "Software";
    ...
    ROS_INFO(
      "    Type                 = %s",
      arv_camera_is_uv_device(p_camera_) ? "USB3Vision" : (arv_camera_is_gv_device(p_camera_) ? "GigEVision" : "Other"));
    ...
    ROS_INFO("    Acquisition Mode     = %s",
             implemented_features_["AcquisitionMode"] ? arv_device_get_string_feature_value(p_device_, "AcquisitionMode") : "(not implemented in camera)");
    ROS_INFO("    Trigger Mode         = %s",
             implemented_features_["TriggerMode"] ? arv_device_get_string_feature_value(p_device_, "TriggerMode") : "(not implemented in camera)");
    ROS_INFO("    Trigger Source       = %s",
             implemented_features_["TriggerSource"] ? arv_device_get_string_feature_value(p_device_, "TriggerSource") : "(not implemented in camera)");
    ...
    if (implemented_features_["BalanceWhiteAuto"])
    {
        ...
        if (awb_mode != "Continuous" && !wb_ratio_selector_feature.empty() && implemented_features_[wb_ratio_selector_feature] && !wb_ratio_feature.empty() && implemented_features_[wb_ratio_feature])
        {
            for (int l = 0; l < std::min(wb_ratio_selector_values.size(), wb_ratio_str_values.size());
                 l++)
            {
...
void CameraAravisNodelet::resetPtpClock()
{
    ...
    const char* ptpS_satus_char_ptr =
      arv_device_get_string_feature_value(p_device_, ptp_status_feature_.c_str());
    std::string ptp_status_str = (ptpS_satus_char_ptr) ? std::string(ptpS_satus_char_ptr) : "Faulty";
...
void CameraAravisNodelet::newBufferReady(ArvStream* p_stream, CameraAravisNodelet* p_can, std::string frame_id, size_t stream_id)
{
    ...
    if (p_buffer != NULL)
    {
        ...
        if (arv_buffer_get_status(p_buffer) == ARV_BUFFER_STATUS_SUCCESS && p_can->p_buffer_pools_[stream_id] && p_can->cam_pubs_[stream_id].getNumSubscribers())
        {
            (p_can->n_buffers_)++;
...
void CameraAravisNodelet::readAndPublishCameraDiagnostics(double rate) const
{
      ...
      -> std::string
    {
        ...
        {
            const char* retFeatVal = arv_device_get_string_feature_value(p_device_, name.c_str());
            valueStr               = (retFeatVal) ? std::string(retFeatVal) : "";
        }
